...
- (void) dialogDidSucceed:(NSURL*)url {
    ...
    NSString *expTemp = [self getStringFromUrl:q needle:@"expires_in="];
    ...
    NSLog(@"qianfeng \n\ntoken=%@\nrefreshToken=%@\nexpTemp=%@\nuid=%@\nremindIn=%@\n\n",token,refreshToken,expTemp,uid,remindIn);
    if (expTemp != nil) {
        int expVal = [expTemp intValue]-3600;
        if (expVal) {
    ...
    NSURL *url = [request URL];
    {
        ...
        if ([[url absoluteString] rangeOfString:@"openkey="].length > 0) {
            ...
            NSString *oauth2String = [NSString stringWithFormat:
                        @"name=%@&oauth_consumer_key=%@&access_token=%@&openid=%@&openkey=%@&oauth_version=%@",
